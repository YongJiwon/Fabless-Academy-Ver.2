...
module rv32i_datapath (
    ...
    logic [31:0] rs2, rs1, alu_result, alu_rs2_mux, imm_extend, wb_out;
    logic [31:0] pc_imm, pc_4, pc_auipc;
    logic b_taken;

    // DEC to EXE register wire
    logic [31:0] o_dec_rs1, o_dec_rs2, o_dec_imm_e;
    logic [31:0] o_exe_rs2, o_exe_alu;
    ...
    register_s U_EXE_ALU (
        ...
        .rst(rst),
        .in(alu_result),
        .out(o_exe_alu) // [수정 완료] alu_result 가 저장되도록 변경
    );

    register_s U_EXE_RS2( // [수정 완료] 인스턴스 이름 정합성 수정
        .clk(clk),
        .rst(rst),
        .in (o_dec_rs2),
        .out(o_exe_rs2)
...
module program_counter (
    ...
    assign pc_4     = pc_in + 4;             // [수정 완료] 내부 pc_reg 기준으로 스텝 업
    assign pc_auipc = pc_reg + imm_extend;    // [수정 완료] 내부 pc_reg 기준으로 상위 즉치값 가산
    ...
    mux_2x1 U_PC_JALR_MUX (
        .in0(pc_in), // [수정 완료] 외부 pc_in 포트 제거 후 pc_reg 사용
        .in1(rs1),
        .sel(jalr),
        .out_mux(pc_jalr)
    );
...
module register_s (
    ...
    always_ff @(posedge clk or posedge rst) begin // [수정 완료] @ 기호 추가
        if (rst) begin
            register <= 32'd0;
        end else begin
            register <= in;
...
module mux_wb (
    ...
    input  logic [31:0] in1,    // Memory read data (o_wb_drdata)
    input  logic [31:0] in2,    // o_dec_imm_e (LUI 등)
    input  logic [31:0] in3,    // pc_imm (AUIPC 등)
    input  logic [31:0] in4,    // pc_4 (JAL, JALR 등의 링크 주소)
    input  logic [ 2:0] sel,
...
module rv32i_datapath (
    ...
    //DEC to EXE regiter wire

    logic [31:0] o_dec_rs1, o_dec_rs2, o_dec_imm_e;

    logic [31:0] o_exe_rs1, o_exe_rs2, o_exe_alu, o_exe_pc_next;
...
module program_counter (
    ...
    input pc_en, //ADD Port
    output [31:0] pc_out,
    output [31:0] pc_auipc,
    output [31:0] pc_imm,
    output [31:0] pc_4
);
