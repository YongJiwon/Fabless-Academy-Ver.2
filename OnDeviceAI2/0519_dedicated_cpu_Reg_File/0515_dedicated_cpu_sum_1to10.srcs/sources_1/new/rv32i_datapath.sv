...
program_counter U_PC(
    .clk(clk),
    .rst(rst),
    .pc_in(instr_addr), //for next program counter
    .pc_out(instr_addr) //for current program counter
);
...
module alu (
    ...
    always_comb begin
        ...
        case(alu_control)
            ...
            `XOR: alu_result = a ^ b;
            `SRL: alu_result = a >> b; //unsigned shift
            `SRA: alu_result = $signed(a) >> b; //signed shift
            `OR: alu_result = a | b;
            `AND: alu_result = a & b;
        endcase
    end
