...
module data_mem (
    ...
    // =========================================================================
    // 1. 저장(Store) 블록: 동기 순차회로 (always_ff) -> 주석 해제 상태 점검 필수
    // =========================================================================
    ...
    // =========================================================================
    // 2. 로드(Load) 블록: 조합회로 (always_comb) -> drdata 명확하게 할당되도록 수정
    // =========================================================================
    logic [31:0] full_word;
    assign full_word = data_ram[daddr[31:2]];

    always_comb begin
        drdata = 32'd0; // latch 방지용 기본 초기화
        
        case (mem_mode)
            // LW (funct3 = 3'b010, `SW와 동일)
            `SW: begin 
                drdata = full_word;
            end
            
            // LB (funct3 = 3'b000, `SB와 동일) -> 부호 확장 로직 명시
            `SB: begin 
                case (daddr[1:0])
